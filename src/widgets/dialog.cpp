...
Dialog::Dialog(ppltk::Window *parent, int width, int height, int buttons) : ppltk::Widget()
{
    ...
    if (parent) {
        create((parent->width() - width) / 2, (parent->height() - height) / 2, width, height);
    } else {
...
void Dialog::mouseDownEvent(ppltk::MouseEvent *event)
{
    ...
    if (widget == ok_button) {
        this->deleteLater();
        return;
    } else if (copy_button && widget == copy_button)
        dialogButtonEvent(Buttons::Copy);
    else if (paste_button && widget == paste_button)
        dialogButtonEvent(Buttons::Paste);
    else if (reset_button && widget == reset_button)
        dialogButtonEvent(Buttons::Reset);
    else if (test_button && widget == test_button)
        dialogButtonEvent(Buttons::Test);
