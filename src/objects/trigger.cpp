...
{
    return Representation(SpritesetId::GenericObjects, 2);
}
...
void Trigger::handleCollision(Player* player, const Collision& collision)
{
    ...
    uint64_t frame_no = collision.clock.frame_count;
...
void Trigger::notifyTargets() const
{
    ...
    for (int i = 0; i < 10; i++) {
        if (triggerObjects[i].object_id > 0) {
            Object* target = objectSystem->getObject(triggerObjects[i].object_id);
            if (target) {
                if (triggerObjects[i].state == Trigger::TargetState::trigger)
                    target->trigger();
                else if (triggerObjects[i].state == Trigger::TargetState::enable)
                    target->toggle(true);
                else if (triggerObjects[i].state == Trigger::TargetState::disable)
                    target->toggle(false);
            }
        }
    }
...
void Trigger::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
    boundary.setRect(p.x - range.x / 2, p.y - range.y / 2, range.x, range.y);
    if (state == State::activated) {
        // ppl7::PrintDebugTime("Trigger::update activated\n");
        triggerDeleayTime = clock.time + triggerDeleay;
        state = State::waiting_for_trigger_delay;
    }
    if (state == State::waiting_for_trigger_delay && clock.time >= triggerDeleayTime) {
        state = State::finished;
        notifyTargets();
    }
    if (state == State::finished && multiTrigger == true && last_collision_time + cooldownUntilNextTrigger < clock.time) {
...
TriggerDialog::TriggerDialog(Trigger* object)
    : Dialog(700, 650, Buttons::OK | Buttons::Test | Buttons::Reset)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        target_state_off[i] =
            new ppltk::RadioButton(50, 0, 60, 30, "off", object->triggerObjects[i].state == Trigger::TargetState::disable);
        target_state_off[i]->setEventHandler(this);
        frame->addChild(target_state_off[i]);
        target_state_trigger[i] =
            new ppltk::RadioButton(100, 0, 90, 30, "trigger", object->triggerObjects[i].state == Trigger::TargetState::trigger);
