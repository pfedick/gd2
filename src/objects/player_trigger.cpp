...
void PlayerTrigger::notifyTargets() const
{
    ...
    for (int i = 0; i < 10; i++) {
        if (triggerObjects[i].object_id > 0) {
            ...
            if (target) {
                if (triggerObjects[i].state == TriggerTarget::State::trigger)
                    target->trigger();
                else if (triggerObjects[i].state == TriggerTarget::State::enable)
                    target->toggle(true);
                else if (triggerObjects[i].state == TriggerTarget::State::disable)
                    target->toggle(false);
            }
        }
    }
...
void PlayerTrigger::trigger(Object* source)
{
    ...
    if (damage_per_second_or_trigger != 0)
        GetGame().getPlayer()->dropHealth((float)damage_per_second_or_trigger, static_cast<Physic::HealthDropReason>(damage_type));
    state = State::activated;
    triggerFlags(player);
    ...
    : Decker::ui::Dialog(700, 700, Buttons::OK | Buttons::Test | Buttons::Reset)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        target_state_off[i] =
            new ppltk::RadioButton(50, 0, 50, 30, "off", object->triggerObjects[i].state == TriggerTarget::State::disable);
        target_state_off[i]->setEventHandler(this);
        frame->addChild(target_state_off[i]);
        target_state_trigger[i] =
            new ppltk::RadioButton(100, 0, 70, 30, "trigger", object->triggerObjects[i].state == TriggerTarget::State::trigger);
