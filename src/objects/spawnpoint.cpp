...
{
    return Representation(SpritesetId::GenericObjects, 108);
}
...
class SpawnPointDialog : public Dialog
{
private:
...
SpawnPointDialog::SpawnPointDialog(SpawnPoint* object)
    : Dialog(640, 480, Buttons::OK | Buttons::Reset | Buttons::Test)
{
...
void SpawnPointDialog::dialogButtonEvent(Dialog::Buttons button)
{
    ...
    if (button == Dialog::Buttons::Reset)
        object->reset();
    else if (button == Dialog::Buttons::Test)
        object->emmitObject();
}
