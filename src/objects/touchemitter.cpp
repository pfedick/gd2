...
public:
    TouchParticle(Type type);
    ~TouchParticle();
    static Representation representation();
    virtual void update(const GameClock& clock, TileTypePlane& ttplane, Player& player) override;
};

class TouchEmitterDialog : public Dialog
{
private:
...
{
    return Representation(SpritesetId::GenericObjects, 13);
}

TouchEmitter::TouchEmitter()
    : Object(Type::TouchEmitter)
{
...
void TouchEmitter::emmitObject(double time)
{
    ...
    // TODO: audio.playOnce(AudioClip::impact1, 0.2f);
    ...
    switch (direction) {
    case 0:
        particle->velocity.setPoint(v0, -v1);
        break;
    case 1:
        particle->velocity.setPoint(v1, v0);
        break;
    case 2:
        particle->velocity.setPoint(v0, v1);
        break;
...
void TouchEmitter::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
    if (((touchtype & 15) == 3 || (touchtype & 15) == 4) && state == State::triggered) {
        if (toogle_count < max_toggles && next_touch_time < clock.time) {
            emmitObject(clock.time);
...
size_t TouchEmitter::save(unsigned char* buffer, size_t size) const
{
    ...
    ppl7::Poke16(buffer + bytes + 3, static_cast<uint16_t>(emitted_object));
...
size_t TouchEmitter::load(const unsigned char* buffer, size_t size)
{
    ...
    emitted_object = static_cast<Type>(ppl7::Peek8(buffer + bytes + 3));
...
TouchParticle::TouchParticle(Type type)
    : Object(Type::Projectile)
{
    ...
    child = objectSystem->getInstance(type);
    child->spawned = true;
}
...
void TouchParticle::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
    p += velocity * clock.frame_rate_compensation;
    if (ppl7::grafix::Distance(p, initial_p) > max_distance) {
        ...
        child->initial_p = p;
        child->spawned = true;
        child->update(clock, ttplane, player);
