...
{
    return Representation(SpritesetId::GenericObjects, 8);
}
...
void ParticleEmitter::createParticle(ParticleSystem* ps, double time)
{
    ...
    if (flags & static_cast<int>(Flags::useScaleGradient)) particle->initScaleGradient(scale_gradient, particle->scale);
...
void ParticleEmitter::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
    if (next_birth < clock.time && current_state == true) {
        ParticleSystem* ps = GetParticleSystem();
        next_birth = clock.time + randf(birth_time_min, birth_time_max);
        if (!emitterInPlayerRange(static_cast<int>(myParallaxLayer), p, player)) return;
        int new_particles = ppl7::rand(min_birth_per_cycle, max_birth_per_cycle);
        for (int i = 0; i < new_particles; i++) {
            createParticle(ps, clock.time);
        }
    }
}
...
size_t ParticleEmitter::load(const unsigned char* buffer, size_t size)
{
    ...
    if (flags & static_cast<int>(Flags::initialStateDisabled))
        current_state = false;
    else
        current_state = true;
...
ppl7::String ParticleEmitter::generateCode() const
{
    ...
    if (scale_gradient.size() > 0 && (flags & static_cast<int>(Flags::useScaleGradient)))
        code += "    std::list<Particle::ScaleGradientItem>scale_gradient;\n";
    if (color_gradient.size() > 0 && (flags & static_cast<int>(Flags::useColorGradient)))
        code += "    std::list<Particle::ColorGradientItem>color_gradient;\n";
    ...
    if (emitter_type == EmitterType::Point) {
        ...
        code.appendf("            particle->p=getBirthPosition(p, %s, ppl7::grafix::Size(%d, %d), %0.3f);\n", (const char*)Tmp,
                     emitter_size.width, emitter_size.height, direction);
    }
    switch (particle_layer) {
    case Particle::Layer::BehindBricks:
        Tmp = "BehindBricks";
        break;
    case Particle::Layer::BehindPlayer:
        Tmp = "BehindPlayer";
        break;
    case Particle::Layer::BeforePlayer:
        Tmp = "BeforePlayer";
        break;
    case Particle::Layer::BackplaneFront:
        Tmp = "BackplaneFront";
        break;
    case Particle::Layer::BackplaneBack:
        Tmp = "BackplaneBack";
        break;
    case Particle::Layer::FrontplaneFront:
        Tmp = "FrontplaneFront";
        break;
    case Particle::Layer::FrontplaneBack:
        Tmp = "FrontplaneBack";
        break;
    default:
        Tmp = "BeforePlayer";
    ...
    case Particle::Type::SoftGradientSmall:
        Tmp = "SoftGradientSmall";
        break;
    case Particle::Type::SoftGradientMedium:
        Tmp = "SoftGradientMedium";
        break;
...
class ParticleEmitterDialog : public Dialog
{
private:
...
ParticleEmitterDialog::ParticleEmitterDialog(ParticleEmitter* object)
    : Dialog(750, 620, Dialog::Buttons::OK | Dialog::Buttons::CopyAndPaste)
{
    this->object = object;
...
void ParticleEmitterDialog::setupParticleTab()
{
    ...
    particle_type->add("rotating transparent particle",
                       ppl7::ToString("%d", static_cast<int>(Particle::Type::RotatingParticleTransparent)));
    particle_type->add("rotating particle", ppl7::ToString("%d", static_cast<int>(Particle::Type::RotatingParticleWhite)));
    particle_type->add("rotating snowflake transparent",
                       ppl7::ToString("%d", static_cast<int>(Particle::Type::RotatingSnowflakeTransparent)));
...
void ParticleEmitterDialog::setupColorTab()
{
    ...
    color = new ColorSliderWidget(col1, y, client.width() - col1, 4 * 35, true);
    ...
    gradient_widget = new GradientWidget(0, y, col3 - 10, 7 * 35);
    ...
    y += 35;
    color_gradient = new ColorSliderWidget(col3, y, client.width() - col3, 4 * 35, true);
...
void ParticleEmitterDialog::setupSizeTab()
{
    ...
    y += 35;
    int col3 = tab->width() - 500;
    scale_gradient_widget = new GradientWidget(0, y, col3 - 10, 7 * 35);
...
void ParticleEmitterDialog::setValuesToUi(const ParticleEmitter* object)
{
...

    {
        ...
        if (object->flags & static_cast<int>(ParticleEmitter::Flags::useColorGradient))
            radio_color_gradient->setChecked(true);
        else
            radio_solid_color->setChecked(true);
