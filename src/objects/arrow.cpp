...
{
    return Representation(SpritesetId::GenericObjects, 109);
}
...
void ArrowFired::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
...
void Arrow::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
    if (!current_state_on) return;
    if (state == 0) {
        double dist = ppl7::grafix::Distance(p, player.position());
        if (next_state < clock.time || (dist < player_activation_distance && min_cooldown_state < clock.time)) {
            state++;
            if (state == 1) {
                next_state = clock.time + ppl7::randf(min_cooldown_time, max_cooldown_time);
                if (GetGame().config.difficulty == Config::DifficultyLevel::easy)
                    min_cooldown_state = clock.time + ppl7::randf(0.600, 1.400);
                else
                    min_cooldown_state = clock.time + ppl7::randf(0.300, 0.800);
                state = 0;
            }
            // Emit a new arrow
            fire();
...
void Arrow::fire()
{
    ...
    switch (direction) {
    case 0:
        particle->velocity.setPoint(0, -24);
        break;
    case 1:
        particle->velocity.setPoint(24, 0);
        break;
    case 2:
        particle->velocity.setPoint(0, 24);
        break;
...
class ArrowDialog : public Dialog
{
private:
