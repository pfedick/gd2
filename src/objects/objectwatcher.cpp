...
{
    return Representation(SpritesetId::GenericObjects, 7);
}

ObjectWatcher::ObjectWatcher()
    : Object(Type::ObjectWatcher)
{
    sprite_set = SpritesetId::GenericObjects;
    sprite_no = 7;
    collisionDetection = false;
    visibleAtPlaytime = false;
    sprite_no_representation = 7;
    for (int i = 0; i < 5; i++) {
...
void ObjectWatcher::update(const GameClock& clock, TileTypePlane& ttplane, Player& player)
{
    if (!enabled) return;
    int maxcount = 0;
    int state_achived = 0;
    for (int i = 0; i < 10; i++) {
        if (watchObjects[i].object_id > 0) {
            Object* obj = objectSystem->getObject(watchObjects[i].object_id);
            if (obj) {
...
class ObjectWatcherDialog : public Dialog
{
private:
...
ObjectWatcherDialog::ObjectWatcherDialog(ObjectWatcher* object)
    : Dialog(700, 650, Buttons::OK | Buttons::Reset | Buttons::Test)
{
