...
static void storeParameters(ppl7::AssocArray& a, const LevelParameter& params)
{
    ...
    if (params.randomSong)
        a.set("randomSong", "true");
    else
        a.set("randomSong", "false");

    if (params.partOfStory)
        a.set("partOfStory", "true");
    else
        a.set("partOfStory", "false");

    if (params.visibleInLevelSelection)
        a.set("visibleInLevelSelection", "true");
    else
        a.set("visibleInLevelSelection", "false");
    ...
    if (params.drainBattery)
        a.set("drainBattery", "true");
    else
        a.set("drainBattery", "false");
    a.setf("batteryDrainRate", "%0.3f", params.batteryDrainRate);

    if (params.flashlightOnOnLevelStart)
        a.set("flashlightOnOnLevelStart", "true");
    else
        a.set("flashlightOnOnLevelStart", "false");
