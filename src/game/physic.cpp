...
void Physic::updateCollisionMatrix(const TileTypePlane& world, float& x, float& y)
{
    ...
    for (int cx = 0; cx < 6; cx++) {
        collision_matrix[cx][0] = filterRelevantTileTypes(
            world.getType(ppl7::grafix::Point(x + (TILE_WIDTH / 2) - 3 * TILE_WIDTH + (cx * TILE_WIDTH), y - (TILE_HEIGHT * 4))));

        collision_matrix[cx][5] =
            filterRelevantTileTypes(world.getType(ppl7::grafix::Point(x + (TILE_WIDTH / 2) - 3 * TILE_WIDTH + (cx * TILE_WIDTH), y)));
    }
}
...
Physic::PlayerMovement Physic::checkCollisionWithWorld(const TileTypePlane& world, float& x, float& y)
{
    ...
    if (collision_at_pivoty[1] == TileType::Plate2h) {
        ...
        if (movement == Falling || movement == Jump || movement == ClimbDown) new_movement = Stand;
        return new_movement;
    } else if (collision_at_pivoty[1] == TileType::Blocking &&
               filterRelevantTileTypes(world.getType(ppl7::grafix::Point(x, y - 1))) == TileType::Blocking) {
    ...
    if (isSwimming()) {
        ...
        if (orientation == PlayerOrientation::Left) {
            ...
            while (collision_matrix[1][2] == TileType::Blocking || collision_matrix[1][3] == TileType::Blocking ||
                   collision_matrix[1][4] == TileType::Blocking) {
                x++;
                updateCollisionMatrix(world, x, y);
            }
            velocity_move.x = 0;
            ...
            while (collision_matrix[4][2] == TileType::Blocking || collision_matrix[4][3] == TileType::Blocking ||
                   collision_matrix[4][4] == TileType::Blocking) {
                x--;
                updateCollisionMatrix(world, x, y);
            }
            velocity_move.x = 0;
