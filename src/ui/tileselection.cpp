...
ColorPaletteFrame::ColorPaletteFrame(int x, int y, int width, int height, ColorPalette& palette)
    : ppltk::Frame(x, y, width, height, ppltk::Frame::BorderStyle::Inset),
      palette(palette)
{
    color_index = 0;
...
void ColorPaletteFrame::mouseDownEvent(ppltk::MouseEvent* event)
{
    if (event->widget() == this) {
        ...
        if (new_index < 256) {
            ...
            } else if (event->buttonMask & ppltk::MouseState::Middle ||
                       (event->buttonMask & ppltk::MouseState::Left && state[SDL_SCANCODE_LSHIFT])) {
