...
LevelDialog::LevelDialog(int width, int height)
    : Dialog(NULL, width, height, Dialog::None)
{
...
void LevelDialog::setupUi()
{
    ...
    cancel_button =
        new ppltk::Button(clientarea.width - 130, clientarea.height - 30, 120, 30, translate("Cancel"), wm->Toolbar.getDrawable(25));
...
void LevelDialog::setupItemsAndOptionsTab()
{
    ...
    /*
    available_items->add("Flashlight", ppl7::ToString("%d", Objects::Type::Flashlight));
...
{
    this->game = game;
    previous_background = game->level.params.BackgroundColor;
}
...
void LevelDialog::loadValues(const LevelParameter& params)
{
    ...
    // Background
    if (params.backgroundType == Background::Type::Color)
        radio_color->setChecked(true);
    else if (params.backgroundType == Background::Type::Image)
...
void LevelDialog::saveValues(LevelParameter& params) const
{
    ...
    for (auto it = translator.languages.begin(); it != translator.languages.end(); ++it) {
        ...
        if (it2 != LevelName.end())
            params.LevelName[it->first] = ppl7::String(it2->second->text()).trimmed();
        else
            params.LevelName[it->first] = "";
        auto it3 = Description.find(it->first);
        if (it3 != Description.end())
            params.Description[it->first] = ppl7::String(it3->second->text()).trimmed();
        else
            params.Description[it->first] = "";
    }
...
void LevelDialog::updateColorPreview()
{
    if (color_preview && color_red && color_green && color_blue) {
        ...
        if (game) game->level.params.BackgroundColor = color_preview->backgroundColor();
    }
}
...
void LevelDialog::mouseClickEvent(ppltk::MouseEvent* event)
{
    if (event->widget() == ok_button) {
        ...
        if (game) {
            game->level.params.BackgroundColor = previous_background;
        }
...
void LevelDialog::mouseDownEvent(ppltk::MouseEvent* event)
{
    ...
    if (w == add_soundtrack_button) {
        soundtrack_list->add(additional_soundtrack->currentText(), additional_soundtrack->currentIdentifier());
        ...
        ppl7::grafix::Image img = game->renderer.getScreenshot(320, 180);
        thumbnail->setIcon(img);
...
void LevelDialog::valueChangedEvent(ppltk::Event* event, double value)
{
    ...
    if (widget == battery_drain_rate) {
        if (value > 0.0f)
            battery_empty_time->setText(ppl7::ToString("empty in %0.0f seconds", 100.0f / value));
        else
            battery_empty_time->setText("disabled");
    }
}
...
void LevelDialog::textChangedEvent(ppltk::Event* event, const ppl7::String& text)
{
    ...
    if (level_width != NULL && level_width != NULL) {
        if (widget == level_width || widget == level_height) {
            level_pixel_size->setText(
                ppl7::ToString("= %d x %d pixel", level_width->value() * TILE_WIDTH, level_height->value() * TILE_HEIGHT));
        }
    }
}
...
void LevelDialog::keyDownEvent(ppltk::KeyEvent* event)
{
    ...
    if ((event->key == ppltk::KeyEvent::KEY_TAB || event->key == ppltk::KeyEvent::KEY_RETURN) &&
        ...
        if (widget == level_width)
            wm->setKeyboardFocus(level_height);
        else if (widget == level_height)
            wm->setKeyboardFocus(color_red);
        else if (widget == color_red)
            wm->setKeyboardFocus(color_green);
        else if (widget == color_green)
            wm->setKeyboardFocus(color_blue);
